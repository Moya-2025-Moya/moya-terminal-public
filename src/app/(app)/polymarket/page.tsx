...
import { PolymarketTerminal } from "@/components/polymarket/PolymarketTerminal";
import type { MarketWithVol } from "@/components/polymarket/MarketSelector";
import { polymarket, isPolymarketConfigured } from "@/lib/polymarket";
...
export default async function PolymarketPage() {
  ...
  // Live terminal goes full-bleed (it carries its own account strip). Only the
  // non-terminal states keep the page header.
  if (!isPolymarketConfigured()) {
    return (
      <div>
        <PageHeader title="Polymarket" />
        <Pending note="Set POLYMARKET_PROXY_URL and INFRA_API_TOKEN to read CLOB via the droplet proxy (:3003)." />
      </div>
    );
  }
  if (error) {
    return (
      <div>
        <PageHeader title="Polymarket" />
        <div className="text-sm text-neg">Couldn&apos;t reach the proxy (:3003) - {error}</div>
      </div>
    );
  }
  if (markets.length === 0) {
  ...
  return <PolymarketTerminal markets={markets} />;
}
