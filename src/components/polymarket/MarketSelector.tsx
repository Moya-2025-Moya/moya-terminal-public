...
type Filter = "all" | "movers" | "watch" | "held" | "alarm";
...
export function MarketSelector({
  ...
  // Per-market alarm state: amber if an alarm is armed, red if one has fired.
  const alarmState = useMemo(() => {
    const m = new Map<string, "armed" | "fired">();
    for (const a of alarms) {
      const cur = m.get(a.conditionId);
      if (a.triggered) m.set(a.conditionId, "fired");
      else if (cur !== "fired") m.set(a.conditionId, "armed");
    }
    return m;
  }, [alarms]);
  ...
  const filtered = useMemo(() => {
    ...
    const out = markets.filter((m) => {
      ...
      if (filter === "alarm" && !alarmState.has(m.condition_id)) return false;
    ...
    return out;
  }, [markets, q, filter, sort, category, watch, held, alarmState]);

  const moversCount = useMemo(() => markets.filter((m) => moverFlags(m).isMover).length, [markets]);
  const counts = { movers: moversCount, watch: watch.size, held: held?.size ?? 0, alarm: alarmState.size };
    ...
    <div className="flex h-full flex-col">
      ...
      <ul className="flex-1">
        {filtered.slice(0, RENDER_CAP).map((m) => {
          ...
          const alarm = alarmState.get(m.condition_id);
          ...
          return (
            <li key={m.condition_id}>
              ...
              >
                ...
                {alarm && (
                  <span
                    className={`shrink-0 h-1.5 w-1.5 rounded-full ${alarm === "fired" ? "bg-neg animate-pulse" : "bg-warn"}`}
                    title={alarm === "fired" ? "Alert fired" : "Alert armed"}
                  />
                )}
