...
import { setOrderDraft } from "@/lib/pm-order-draft";
...
export function MarketView({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Order books - click any level to prefill the order ticket */}
      <div className="grid grid-cols-1 gap-x-10 gap-y-6 sm:grid-cols-2">
        {market.tokens.map((t) => (
          <DepthLadder key={t.token_id} book={books[t.token_id]} label={t.outcome} tokenId={t.token_id} />
        ))}
      </div>
      ...
      {/* Price/probability alerts for this market */}
      <AlarmPanel market={market} />
      ...
      {/* Position in this market - only renders if you hold one */}
      <SpotPositions conditionId={market.condition_id} enableClose />
    </div>
  );
}
...
function DepthLadder({ book, label, tokenId }: { book: PmBook | null; label: string; tokenId: string }) {
  ...
  // Clicking an ask = you buy at that price; a bid = you sell at that price.
  const pick = (p: number, s: number, side: "bid" | "ask") =>
    setOrderDraft({ tokenId, side: side === "ask" ? "buy" : "sell", price: p, size: s });
      ...
      {!book || (!asks.length && !bids.length) ? (
        ...
        <div className="font-mono text-xs">
          {[...asks].reverse().map((a, i) => (
            <Row key={`a${i}`} price={a.p} size={a.s} max={maxSize} side="ask" onPick={pick} />
          ))}
          <div className="my-0.5 border-y border-hairline" />
          {bids.map((b, i) => (
            <Row key={`b${i}`} price={b.p} size={b.s} max={maxSize} side="bid" onPick={pick} />
          ))}
        </div>
      )}
    </div>
...
function Row({
  ...
  max: number;
  side: "bid" | "ask";
  onPick: (price: number, size: number, side: "bid" | "ask") => void;
}) {
  ...
  return (
    <button
      onClick={() => onPick(price, size, side)}
      title="Click to fill the order ticket"
      className="relative flex w-full items-center justify-between py-0.5 hover:bg-elevated/60"
    >
      <div
